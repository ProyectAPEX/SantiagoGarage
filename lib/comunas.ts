import { filtrar, type Sugerencia } from "@/lib/sugerencias";

/**
 * Comunas de la Region Metropolitana, para el domicilio del cliente. El campo
 * igual es libre: si el auto viene de otra region, se escribe y listo.
 */
...
export function buscarComunas(texto: string): Sugerencia[] {
  return filtrar(
    COMUNAS.map((c) => ({ texto: c })),
    texto,
    6
  );
}
